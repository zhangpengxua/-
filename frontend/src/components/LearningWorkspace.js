...
export default function LearningWorkspace({conversations,currentConversation,onSelect,onNew,onDelete,onSend,isLoading,thinkingState,onStop,ocrResult,error}) {
 ...
 useEffect(()=>{const observer=new ResizeObserver(([entry])=>setSize({width:Math.floor(entry.contentRect.width),height:Math.floor(entry.contentRect.height)}));if(sceneRef.current)observer.observe(sceneRef.current);return()=>observer.disconnect();},[]);
 ...
 <div className={`panels ${wide?'scene-wide':''}`} ref={layoutRef} style={{'--study-width':`${split}%`}}><section className="study-panel"><div className="panel-heading"><h2>理解这道题</h2><span className="progress">{String(steps.length?active+1:0).padStart(2,'0')} <em>/ {String(steps.length).padStart(2,'0')}</em></span></div>
 <div className="study-scroll"><div className="steps">{steps.map((item,index)=><article className={`step ${active===index?'active':''}`} key={index}><button className="step-heading" aria-expanded={active===index} onClick={()=>setActive(index)}><span className="step-number">{index<active?'✓':String(index+1).padStart(2,'0')}</span><strong>{item.title||`步骤 ${index+1}`}</strong><span>{active===index?'⌃':'›'}</span></button>{active===index&&<div className="step-body"><Md>{item.description}</Md>{(demo||item.drawingData)&&<span className="sync-chip">↔ 与右侧场景同步</span>}</div>}</article>)}</div>
 ...
 <div className="scene-content" ref={sceneRef}>{demo?<Cone height={height} reset={reset} onAsk={ask} highlight={highlight}/>:step?.drawingData?(step.is2DPlot||['MATH_STATIC_EQUATION','MATH_STATIC_2D_FUNCTION'].includes(step.imageType)?<FunctionPlot drawingData={step.drawingData} width={Math.max(200,size.width)} height={Math.max(200,size.height)}/>:<Interactive3DViewer key={`${active}-${reset}`} drawingData={step.drawingData} description={step.description} imageType={step.imageType}/>):<div className="scene-empty"><Icon name="book"/><h3>专注当前推理</h3><p>这一步暂无可视化数据。<br/>阅读左侧讲解，或针对不理解的地方追问。</p></div>}</div><div className="scene-hint">{demo?'鼠标拖动旋转 · 滚轮缩放 · 点击截面追问':'选择左侧步骤，查看对应图形'}</div></div>
